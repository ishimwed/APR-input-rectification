...
let write_file_bin (filename:string) content: unit = (* writes content/bytes to a file *)
  let fout = open_out_bin filename in
  Marshal.to_channel fout content [];
  close_out fout;
  P.printf "write_file_bin: '%s'\n%!" filename

let read_file_bin (filename:string) = (* Reads content/bytes from a file *)
...
let mkUk
  ...
  let mkCoe
    ...
    let vname = "c_" ^ string_of_int uid in
    let vi:varinfo = makeLocalVar mainFd vname utyp in
    let lv:lval = var vi in
